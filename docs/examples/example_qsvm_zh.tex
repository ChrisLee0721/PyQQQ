...
量子 SVM 可以用于金融欺诈检测。交易数据通常是高维的，量子核函数可以在更高维的空间中捕捉复杂的欺诈模式。

\subsection{药物发现}

量子 SVM 可以用于药物分子活性预测。分子描述符是高维稀疏向量，量子核函数可以有效捕捉分子结构的细微差异。

\subsection{图像识别}

量子 SVM 可以用于小样本图像分类。在标注数据稀缺的场景下（如医学图像），量子核函数可以在有限数据上达到更好的分类精度。

\subsection{材料科学}

量子 SVM 可以用于材料性质预测。材料的电子结构可以用高维特征向量表示，量子核函数可以捕捉材料结构与性质之间的复杂映射。
