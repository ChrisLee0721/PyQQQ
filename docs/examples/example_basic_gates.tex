...
基本量子门是所有量子算法的基础。例如，Grover 搜索需要 Hadamard 门和 Oracle 门，Shor 算法需要 QFT 门和受控相位门。理解基本量子门是实现任何量子算法的前提。

\subsection{量子态制备}

量子门用于制备特定的量子态。例如，制备 Bell 态需要 Hadamard 门和 CNOT 门，制备 GHZ 态需要 Hadamard 门和多个 CNOT 门。量子态制备是量子通信、量子密码学的基础。

\subsection{量子纠错}

量子门用于实现量子纠错码。例如，比特翻转码需要 CNOT 门和测量门，Shor 码需要多个 CNOT 门和 Hadamard 门。量子纠错是实现容错量子计算的关键。

\subsection{量子模拟}

量子门用于模拟量子系统。例如，模拟分子的电子结构需要旋转门和受控门，模拟量子多体系统需要 Trotter 分解和基本量子门。量子模拟是量子计算的重要应用之一。
