...
扩散算子是 Grover 搜索的核心组件，负责放大目标态的振幅。Grover 搜索用于在无序数据库中查找目标，提供二次加速。

\subsection{量子机器学习}

扩散算子可以用于量子玻尔兹曼机、量子支持向量机等量子机器学习算法中的采样和优化。

\subsection{量子优化算法}

扩散算子可以用于 QAOA 等量子优化算法，作为混合算子在解空间中搜索最优解。

\subsection{量子计数}

扩散算子可以用于量子计数算法，估计满足条件的解的数量。经典方法需要遍历所有记录，量子计数只需要 $O(\sqrt{N})$ 次查询。
